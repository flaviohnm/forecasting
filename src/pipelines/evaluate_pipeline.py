...
def run(main_config, model_conf, dataset_conf, exec_name):
    ...
    diag_path = main_config['results_paths'].get('diagnostics', os.path.join(os.path.dirname(metrics_path), 'diagnostics'))
    ...
    metrics_df = pd.DataFrame([{'model': exec_name, 'dataset': dataset_conf['name'], 'horizon': horizon, 'mape': mape, 'mase': mase}])

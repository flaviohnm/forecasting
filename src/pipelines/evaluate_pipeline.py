...
def run(main_config: dict, model_conf: dict, dataset_conf: dict,
    ...
    if model_type == 'ARIMA':
        ...
        final_forecast = pd.Series(
            arima_instance.predict(n_periods=len(test_series)),
        ...
        final_forecast = pd.Series(ets_model.load_and_forecast_ets(
        ...
        model_map = {
            'Transformer': (iTransformer, model_conf['transformer_params']),
            'NHiTS': (NHITS, model_conf['nhits_params'])
        }
    ...
    elif model_type in ['Hybrid_Direct', 'Hybrid_MIMO']:

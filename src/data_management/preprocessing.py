...
def load_and_prepare_data(main_config: dict, dataset_conf: dict):
    ...
    """
    file_path = os.path.join(main_config['data_paths']['raw'],
                             dataset_conf['filename'])

    df = pd.read_csv(file_path)

    date_col = dataset_conf['date_column']
    df[date_col] = pd.to_datetime(df[date_col],
                                  format=dataset_conf.get('date_format'))
...
def create_direct_forecast_datasets(series: pd.Series, input_lags: int,
    ...
    for i in range(len(series) - input_lags - forecast_horizon + 1):
        ...
        y_all.append(series.iloc[i + input_lags:i + input_lags +
...
def create_mimo_forecast_dataset(series: pd.Series, input_lags: int,
                                 forecast_horizon: int):
    """
    ...
    for i in range(len(series) - input_lags - forecast_horizon + 1):
        X.append(series.iloc[i:i + input_lags].values)
        y.append(series.iloc[i + input_lags:i + input_lags +
                             forecast_horizon].values)

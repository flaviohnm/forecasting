...
def train_stats_model(df, model_conf, horizon, freq):
    ...
    if "unique_id" not in df.columns:
        if df.index.name == "unique_id":
            ...
            raise ValueError(
                f"DataFrame recebido para treino do {model_type} não possui coluna 'unique_id'. Colunas: {df.columns.tolist()}"
            )
    ...
    if model_type == "ARIMA":
        ...
        model = AutoETS(season_length=params.get("m", 24), model=params.get("model", "ZZZ"))
    ...
    try:
        ...
        logging.error(
            f"Erro interno no .fit() do StatsForecast para {model_type}. Columns: {df.columns}. Dtypes: {df.dtypes}"
        )
        raise e

# /forecasting/src/reporting.py (VERSÃO COM CORREÇÃO DE CAMINHOS)
import pandas as pd
import numpy as np
...
def evaluate_forecasts(forecast_path: str, train_path: str, results_dir: str, target_column: str):
    """Calcula e salva um conjunto de métricas de erro."""
    forecast_df = pd.read_csv(forecast_path)
    train_df = pd.read_csv(train_path)
    
    # --- MUDANÇA AQUI: Extraindo o nome do modelo de forma mais robusta ---
    # Ex: 'airline_ARIMA_forecasts.csv' -> 'ARIMA'
    model_name = Path(forecast_path).stem.split('_')[1]
    ...
    results_path_dir = Path(results_dir)
    results_path_dir.mkdir(parents=True, exist_ok=True)
    
    # Salva as métricas em um arquivo JSON nomeado corretamente
    results_file = results_path_dir / f"{model_name}_metrics.json"
...
def generate_final_report(results_dir: str, dataset_name: str, forecast_horizon: int, model_names: list):
    """Agrega todas as métricas e previsões para gerar um relatório final."""
    print("\n[ETAPA FINAL] Gerando relatório consolidado...")
    
    metrics_dir = Path(results_dir) / "metrics" / dataset_name
    all_metrics = []
    
    # --- MUDANÇA AQUI: Procurando pelo nome de arquivo correto ---
    for model_name in model_names:
        metric_file = metrics_dir / f"{model_name}_metrics.json" # O nome agora bate
